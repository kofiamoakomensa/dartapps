...
void displayBalances() {
  users.forEach((userId, userInfo) {
    ...
    userInfo['transferred'].forEach((shopId, transferredAmount) {
      double spentInShop = userInfo['purchases']
          .where((purchase) => purchase['shopId'] == shopId)
          .fold(0.0, (sum, purchase) => sum + purchase['price']);
      double remainingBalanceInShop = transferredAmount - spentInShop;
      print("    In ${shops[shopId]!['name']}: Transferred \$transferredAmount, Spent \$spentInShop, Remaining \$remainingBalanceInShop");
    });
